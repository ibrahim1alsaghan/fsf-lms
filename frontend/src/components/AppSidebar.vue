<template>
	...
	>
		...
		<div class="m-2 flex flex-col gap-1">
			...
			/>
			<SidebarLink
				v-if="currentUserType === 'System User'"
				:link="{
					label: currentLang === 'ar' ? 'الانظمة الإدارية' : 'Enterprise Systems',
				}"
				:isCollapsed="sidebarStore.isSidebarCollapsed"
				@click="goToDesk"
				>
				<template #icon>
					<span class="grid h-5 w-6 flex-shrink-0 place-items-center">
					<!-- Use Monitor or Layout icon for "Desk" -->
					<Monitor class="h-4 w-4 stroke-1.5 text-ink-gray-7" />
					</span>
				</template>
			</SidebarLink>
			<SidebarLink
				:link="{
					label: currentLang === 'ar' ? 'EN' : 'عربي',
				}"
				:isCollapsed="sidebarStore.isSidebarCollapsed"
				@click="switchLang"
				>
				<template #icon>
					<span class="grid h-5 w-6 flex-shrink-0 place-items-center">
					<!-- Globe for language switch -->
					<Globe class="h-4 w-4 stroke-1.5 text-ink-gray-7" />
					</span>
				</template>
				</SidebarLink>
			<SidebarLink
			...
			<SidebarLink
				:link="{
					label: sidebarStore.isSidebarCollapsed ? (currentLang === 'ar' ? 'توسيع' : 'Expand') : (currentLang === 'ar' ? 'اغلاق' : 'Close'),
				}"
				:isCollapsed="sidebarStore.isSidebarCollapsed"
				@click="toggleSidebar()"
			>
...
import { computed, ref, onMounted, inject, watch, reactive, markRaw, h } from 'vue'
...
const sidebarLinks = computed(() => {
	...
	// Always show Courses
	allLinks.push({
		label: currentLang.value === 'ar' ? 'الدورات' : 'Courses',
		icon: 'BookOpen',
		to: 'Courses',
		activeFor: [
			'Courses',
	...
	// Show Programs when learning paths are enabled OR for instructors/moderators
	const showPrograms = settingsStore.learningPaths.data || isInstructor.value || isModerator.value
	if (showPrograms) {
		allLinks.push({
			label: currentLang.value === 'ar' ? 'برامج' : 'Programs',
			icon: 'Route',
			to: 'Programs',
			activeFor: ['Programs', 'ProgramForm', 'ProgramDetail'],
			key: 'programs',
		})
	}
	...
	allLinks.push(
		{
			label: currentLang.value === 'ar' ? 'الدفعات' : 'Batches',
			icon: 'Users',
			to: 'Batches',
		...
		{
			label: currentLang.value === 'ar' ? 'المشاركون المعتمدون' : 'Certified Participants',
			icon: 'GraduationCap',
		...
		{
			label: currentLang.value === 'ar' ? 'وظائف' : 'Jobs',
			icon: 'Briefcase',
			to: 'Jobs',
		...
		{
			label: currentLang.value === 'ar' ? 'إحصائيات' : 'Statistics',
			icon: 'TrendingUp',
			to: 'Statistics',
	...
	// Add Notifications link if user is logged in
	if (user) {
		allLinks.push({
			label: currentLang.value === 'ar' ? 'إشعارات' : 'Notifications',
			icon: 'Bell',
			to: 'Notifications',
	...
	// Add Quizzes link if user is instructor or moderator
	if (isInstructor.value || isModerator.value) {
		allLinks.push({
			label: currentLang.value === 'ar' ? 'إختبارات' : 'Quizzes',
			icon: 'CircleHelp',
			to: 'Quizzes',
			activeFor: [
				'Quizzes',
	...
	// Add Assignments link if user is instructor or moderator
	if (isInstructor.value || isModerator.value) {
		allLinks.push({
			label: currentLang.value === 'ar' ? 'تكاليف' : 'Assignments',
			icon: 'Pencil',
			to: 'Assignments',
			activeFor: [
				'Assignments',
	...
	return allLinks.filter(link => enabledLinks.value[link.key] !== false)
})
...
// These functions are no longer needed as the links are now part of the computed property
...
onMounted(async () => {
  try {
    ...
    console.log('Initial language loaded:', currentLang.value)
    ...
    const userTypeRes = await fetch('/api/method/lms.lms.utils.get_current_user_type')
    const userTypeData = await userTypeRes.json()
    currentUserType.value = userTypeData.message.user_type
    console.log('User type loaded:', currentUserType.value)
  } catch (err) {
...
const canSeeDesk = createResource({
  url: '/api/method/lms.lms.utils.is_system_user',
  auto: true,
})

const homeLink = computed(() => ({
  label: currentLang.value === 'ar' ? 'الصفحة الرئيسية' : 'Home',
  icon: 'Home',
  to: 'Home',
  activeFor: ['Home'],
}))
