...
//send to instructor function
const messageInstructor = () => {
	if (!props.course.data?.membership) return

	const instructor = props.course.data.instructors?.[0]
	const instructorEmail = instructor?.email || ''

	if (!instructorEmail) {
		showToast('Instructor email not available', '', 'alert-triangle')
		return
	}

	const subject = `Help needed with the course "${props.course.data.name}"`
	const body = `Hi ${instructor.full_name || 'Instructor'},\n\nI'm currently enrolled in "${props.course.data.name}" and I need some help or clarification.\n\n[Write your question here]\n\nThank you!`
