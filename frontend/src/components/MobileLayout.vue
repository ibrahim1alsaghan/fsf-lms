...
const addOtherLinks = () => {
	if (user) {
		otherLinks.value.push({
			label: currentLang.value === 'ar' ? 'الإشعارات' : 'Notifications',
			icon: 'Bell',
			to: 'Notifications',
		})
		otherLinks.value.push({
			label: currentLang.value === 'ar' ? 'الملف الشخصي' : 'Profile',
			icon: 'UserRound',
		})
		otherLinks.value.push({
...
const addQuizzes = () => {
	otherLinks.value.push({
		label: currentLang.value === 'ar' ? 'الاختبارات' : 'Quizzes',
		icon: 'CircleHelp',
		to: 'Quizzes',
	})
}
...
const handleClick = (tab) => {
	if (tab.label == (currentLang.value === 'ar' ? 'تسجيل الدخول' : 'Log in')) window.location.href = '/login'
	else if (tab.label == (currentLang.value === 'ar' ? 'تسجيل الخروج' : 'Log out'))
		logout.submit().then(() => {
			isLoggedIn = false
		})
	else if (tab.label == (currentLang.value === 'ar' ? 'الملف الشخصي' : 'Profile'))
		router.push({
			name: 'Profile',
			params: {
...
const isVisible = (tab) => {
	if (tab.label == (currentLang.value === 'ar' ? 'تسجيل الدخول' : 'Log in')) return !isLoggedIn
	else if (tab.label == (currentLang.value === 'ar' ? 'تسجيل الخروج' : 'Log out')) return isLoggedIn
	else return true
}
</script>
