...
// Note: Chapter Reference is now automatically created by the after_insert hook
// in CourseChapter, so we don't need to create it manually here

const addChapter = async (close) => {
	// Prevent multiple submissions
	if (isSubmitting.value || chapterResource.loading) {
		return
	}
	...
	chapterResource.submit(
		{},
		{
			...
			onSuccess: (data) => {
				...
				// Chapter Reference is automatically created by after_insert hook
				cleanChapter()
