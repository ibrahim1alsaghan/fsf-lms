	...
	<div v-if="lesson.data" class="">
		...
		<div class="grid md:grid-cols-[70%,30%] h-screen">
			...
			<div class="sticky top-10">
				...
				<CertificationLinks :courseName="courseName" class="mt-4" />
...
const lesson = createResource({
	...
	onSuccess(data) {
		...
		hasVideoBlock.value = detectVideoBlock(data?.content)
		if (hasVideoBlock.value) {
...
const detectVideoBlock = (content) => {
    try {
        const parsed = content ? JSON.parse(content) : null
        const blocks = parsed?.blocks || []
        return blocks.some(
            (b) => b?.type === 'upload' && isVideoType(b?.data?.file_type)
        )
    } catch (e) {
        return false
    }
...
const attachVideoEndedListener = () => {
    if (videoEndedListenerAttached) return
    window.addEventListener('lms-video-ended', onVideoEnded, { once: true })
    videoEndedListenerAttached = true
}

const detachVideoEndedListener = () => {
    if (!videoEndedListenerAttached) return
    window.removeEventListener('lms-video-ended', onVideoEnded)
    videoEndedListenerAttached = false
}

const teardownProgressHandlers = () => {
    clearInterval(timerInterval)
    timer.value = 0
    detachVideoEndedListener()
    hasVideoBlock.value = false
}
...
/* Ensure lesson content doesn't interfere with modals */
.ProseMirror,
.codex-editor,
.ce-block,
.ce-block__content {
