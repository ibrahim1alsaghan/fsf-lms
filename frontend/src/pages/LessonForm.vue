			...
			<div class="border-r">
				...
				>
					...
					<Button
						...
						class="mt-3 md:mt-0"
						:loading="isSubmitting || newLessonResource.loading || editLesson.loading"
						:disabled="isSubmitting || newLessonResource.loading || editLesson.loading"
					>
						{{ __('Save') }}
					</Button>
				</header>
...
const lessonDetails = createResource({
	...
	auto: true,
	onSuccess(data) {
		// Set lesson editor context for both new and existing lessons
		window.__lessonEditorContext = {
			course: props.courseName,
			chapter: data.chapter?.name || null,
		}
...
onBeforeUnmount(() => {
	...
	window.__lessonEditorContext = null
})
...
const saveLesson = (e) => {
	...
	if (isSubmitting.value || newLessonResource.loading || editLesson.loading) {
		return
	}
...
const createNewLesson = () => {
	newLessonResource.submit(
		{},
		{
			...
			onSuccess(data) {
				// Lesson Reference is automatically created by after_insert hook
