<template>
	...
	>
		...
		<template #body-content>
			<div class="text-base">
				<div class="grid grid-cols-1 md:grid-cols-2 gap-5 pb-3">
					<FormControl
				...
				<div class="pb-5">
					...
					>
						<ListHeader
							class="mb-2 grid items-center gap-4 rounded bg-surface-gray-2 px-4 py-3"
						>
						...
						<ListRows>
							...
							>
								<template #item="{ element: row }">
									<ListRow :row="row" class="py-3">
										<template #default="{ column, item }">
											<ListRowItem :item="item" :align="column.align" class="px-3">
											</ListRowItem>
										</template>
									</ListRow>
								</template>
				...
				<div>
					...
					>
						...
						<ListRows>
							<ListRow :row="row" v-for="row in program.program_members" class="py-3">
								<template #default="{ column, item }">
									<ListRowItem :item="item" :align="column.align" class="px-3">
									</ListRowItem>
								</template>
...
import { Programs, Program, ProgramCourse, ProgramMember } from '@/types/programs'
import { escapeHTML } from '@/utils'
import Link from '@/components/Controls/Link.vue'
import Draggable from 'vuedraggable'
import ProgramProgressSummary from './ProgramProgressSummary.vue'
...
const openSettings = (tab: string, close: () => void) => {
	close()
...
const addCourse = (close: () => void) => {
	...
	if (!existingCourse) {
		// Add a local row immediately so UI updates without needing reopen.
		// For existing programs, ListView uses `rowKey="name"` and displays `course_title`.
		// So we add a temporary name and fetch the course title to avoid an empty row.
		const idx = program.value.program_courses.length + 1
		// Frappe treats child rows with name starting with "new-" as new rows on save
		const tempName = `new-${Math.random().toString(36).slice(2, 10)}`

		const newRow: any = {
			name: tempName,
			course: course.value,
			course_title: course.value, // fallback until we fetch title
			idx,
		}
		program.value.program_courses.push(newRow)

		call('frappe.client.get_value', {
			doctype: 'LMS Course',
			fieldname: 'title',
			filters: { name: course.value },
		})
			.then((data: any) => {
				if (data?.title) newRow.course_title = data.title
			})
			.catch(() => {
				// keep fallback
			})
...
const updateOrder = async (e: any) => {
	let sourceIdx = e.from?.dataset?.idx || e.oldIndex
	let targetIdx = e.to?.dataset?.idx || e.newIndex
