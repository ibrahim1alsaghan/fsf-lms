...
class LMSQuiz(Document):
	def validate(self):
		...
		self.validate_unique_course_chapter()
	...
	def validate_unique_course_chapter(self):
		"""Validate that each course chapter can only be associated with one quiz."""
		if not self.course_chapter:
			return
		
		# Check if another quiz already uses this course_chapter
		existing_quiz = frappe.db.exists(
			"LMS Quiz",
			{
				"course_chapter": self.course_chapter,
				"name": ["!=", self.name]  # Exclude current quiz
			}
		)
		
		if existing_quiz:
			existing_quiz_title = frappe.db.get_value("LMS Quiz", existing_quiz, "title")
			chapter_title = frappe.db.get_value("Course Chapter", self.course_chapter, "title")
			frappe.throw(
				_(
					"Course Chapter '{0}' is already associated with quiz '{1}'. "
					"Each course chapter can only be associated with one quiz."
				).format(
					frappe.bold(chapter_title or self.course_chapter),
					frappe.bold(existing_quiz_title or existing_quiz)
				)
			)
