...
def get_lesson_details(chapter, progress=False):
	...
	for row in lesson_list:
		...
		# Skip if lesson doesn't exist (might have been deleted)
...
def get_lesson_icon(body, content):
	if content:
		try:
			...
			# If content is not valid JSON, skip JSON parsing
			content = None

	if content:
		for block in content.get("blocks", []):
			block_data = block.get("data") or {}
			if block.get("type") == "upload" and block_data.get("file_type"):
				if block_data.get("file_type").lower() in [
					"mp4",
					"webm",
					"ogg",
					"mov",
			...
			if block.get("type") == "embed" and block_data.get("service") in [
				"youtube",
				"vimeo",
			]:
				return "icon-youtube"
...
def get_course_completion_data():
	...
	return {
		"labels": [_("Completed"), _("In Progress")],
		"datasets": [
			{
				"name": _("Course Completion"),
...
def get_course_outline(course, progress=False):
	...
	for chapter in chapters:
		...
		# Skip if chapter doesn't exist (might have been deleted)
...
@frappe.whitelist()
def get_recommended_quiz_for_chapter(chapter):
	"""Return the recommended quiz for a given course chapter."""
	if not chapter:
		return None
	
	quiz = frappe.get_all(
		"LMS Quiz",
		filters={"course_chapter": chapter},
		fields=["name", "title"],
		limit=1,
	)
...
@frappe.whitelist()
def get_quiz_options(txt=None, chapter=None, page_length=20):
	"""Return quiz options for the lesson editor with chapter info."""
	...
	quiz_names = [result.get("value") for result in results]
	chapter_map = {}

	if quiz_names:
		for quiz in frappe.get_all(
			"LMS Quiz",
			filters={"name": ("in", quiz_names)},
			fields=["name", "course_chapter", "title"],
		):
			chapter_map[quiz.name] = quiz.course_chapter

	options = []
	for result in results:
		name = result.get("value")
		is_chapter_quiz = bool(chapter and chapter_map.get(name) == chapter)
		options.append(
			{
				"label": result.get("label") or result.get("value"),
				"value": name,
				"description": result.get("description"),
				"is_chapter_quiz": is_chapter_quiz,
				"course_chapter": chapter_map.get(name),
			}
		)

	# Sort so chapter quizzes appear first
	options.sort(key=lambda option: option["is_chapter_quiz"], reverse=True)
	return options
...
def update_user_language(language=None):
    if not language:
        frappe.throw("Missing language")

    user = frappe.session.user
    if user and user != "Guest":
        frappe.db.set_value("User", user, "language", language)
        frappe.local.lang = language
        frappe.session.data["lang"] = language
        frappe.local.cookie_manager.init_cookies()
        frappe.db.commit()
        print("✅ Language set to", language)
        return {"status": "success", "new_lang": language}
    else:
        frappe.throw("Cannot update language for Guest")

@frappe.whitelist()
def get_current_user_language():
    user = frappe.session.user
    lang = frappe.db.get_value("User", user, "language")
    return {"user": user, "language": lang or "en"}
